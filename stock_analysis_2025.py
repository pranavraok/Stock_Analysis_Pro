...
# =====================================================
# PROFESSIONAL PDF CLASS - ENHANCED WITH BETTER SPACING
# =====================================================
...
class ProfessionalStockPDF(FPDF):
    ...
    def footer(self):
        ...
        self.cell(0, 10, page_text, 0, 0, 'R')  # Changed to 'R' for right alignment
...
class EnhancedStockAnalyzer:
    ...
    def fetch_stock_info(self):
        try:
            ...
            print(f"Limited info available")
            self.stock_info = {}
            return True
    ...
    def analyze_rsi(self):
        ...
        try:
            ...
            if pd.isna(final_rsi):
                raise ValueError("RSI NaN")
            
            if final_rsi <= 20:
                is_good = True
                recommendation = "Extremely Oversold"
                reasoning = "RSI below 20 indicates extreme oversold condition - potentially strong recovery signal. This represents a major buying opportunity as the stock is severely depressed."
            elif final_rsi <= 30:
                is_good = True
                recommendation = "Very Oversold - Strong Buy Signal"
                reasoning = "RSI below 30 indicates oversold condition - stock likely to recover. This is an excellent entry point for long-term investors as selling pressure has been exhausted."
            elif final_rsi <= 45:
                is_good = True
                recommendation = "Oversold - Buy Signal"
                reasoning = "RSI in 30-45 range indicates downward pressure exhaustion - good accumulation zone. Market is showing weakness but stabilizing, making it suitable for value-oriented traders."
            elif final_rsi <= 55:
                is_good = None
                recommendation = "Neutral Zone"
                reasoning = "RSI 45-55 indicates no extreme momentum - neither overbought nor oversold. Monitor price action and volume for directional clarity before taking positions."
            elif final_rsi <= 70:
                is_good = False
                recommendation = "Overbought - Caution Zone"
                reasoning = "RSI above 55 indicates upward momentum exhaustion - potential pullback ahead. Consider taking profits on existing positions or waiting for better entry points."
            else:
                is_good = False
                recommendation = "Extremely Overbought - Sell Signal"
                reasoning = "RSI above 70 indicates extremely overbought condition with strong probability of reversal. Existing holders should consider profit-taking; new buyers should wait for correction."
    ...
    def analyze_pe_ratio(self):
        ...
        try:
            ...
            if pe_ratio is None or pd.isna(pe_ratio) or pe_ratio <= 0:
                ...
                reasoning = "P/E below 10 indicates exceptional valuation - stock trading significantly below earnings. This suggests strong fundamental value with margin of safety."
            elif pe_ratio < 15:
                is_good = True
                recommendation = "Undervalued - Buy"
                reasoning = "P/E 10-15 indicates good valuation - stock offers value compared to earnings. Attractive for investors seeking quality stocks at reasonable prices."
            elif pe_ratio < 20:
                is_good = True
                recommendation = "Fair Valuation - Good Value"
                reasoning = "P/E 15-20 indicates fair valuation - reasonable price-to-earnings ratio. Normal valuation for stable, quality companies with moderate growth."
            elif pe_ratio < 30:
                is_good = None
                recommendation = "Slightly Expensive"
                reasoning = "P/E 20-30 indicates moderate premium - normal for growing companies. Reflects market expectations for future growth; monitor earnings growth."
            elif pe_ratio < 40:
                is_good = False
                recommendation = "Expensive - Caution"
                reasoning = "P/E above 30 indicates significant premium - limited margin of safety. Stock is pricing in strong future growth; any disappointment could lead to correction."
            else:
                is_good = False
                recommendation = "Severely Overvalued"
                reasoning = "P/E above 40 indicates extreme premium - high risk of correction. Excessive valuation leaves limited room for error; avoid until revaluation."
        ...
        try:
            ...
            if len(revenue_list) >= 2:
                ...
                if revenue_trend and net_profit_trend and revenue_growth > 5:
                    ...
                    reasoning = f"Revenue up {revenue_growth:.1f}% and profit growing - company expanding profitably with improving operational efficiency and bottom-line growth."
                elif revenue_trend and net_profit_trend:
                    is_good = True
                    recommendation = "Improving Fundamentals"
                    reasoning = "Both revenue and net profit increasing - positive business momentum with healthy growth trajectory across all metrics."
                elif revenue_trend:
                    is_good = None
                    recommendation = "Mixed Signals"
                    reasoning = "Revenue growing but net profit concerns - topline expansion not translating to bottom line; profitability under pressure."
                else:
                    is_good = False
                    recommendation = "Declining Fundamentals"
                    reasoning = "Revenue declining - business facing headwinds with contracting sales indicating market share loss or sector challenges."
            else:
                is_good = None
                recommendation = "Insufficient Data"
                reasoning = "Limited historical quarterly data available - requires more quarters for meaningful trend analysis."
    ...
    def generate_verdict(self):
        ...
        try:
            ...
            if ath_analysis.get('is_good') is True:
                signals.append(f"Valuation: {ath_analysis.get('discount_percentage', 0):.1f}% discount from ATH - good entry point")
                positive_count += 1
    ...
    def create_charts(self):
        ...
        try:
            ...
            if fund_data.get('revenue_list') and fund_data.get('dates'):
                ...
                if len(revenue_list) > 0:
                    ...
                    ax.set_title('Quarterly Financial Performance - Revenue, Operating Profit, Net Profit (Crores)', fontsize=16, fontweight='bold', pad=20)
    ...
    def generate_professional_pdf(self):
        """Generate comprehensive professional PDF report with improved spacing"""
        try:
            ...
            # Add separator line after Company Overview section
            ...
            # Add separator line after Key Metrics section
            pdf.add_section_separator()
            ...
            # Add separator line after 52-Week Performance section
            pdf.add_section_separator()
            ...
            trend_reason = "Stock trading above both key moving averages - strong uptrend with sustained bullish momentum and healthy technical structure" if trend_good is True else "Stock below key moving averages - weak downtrend with bearish momentum and deteriorating technical position" if trend_good is False else "Stock near moving averages - mixed signals requiring clarity before taking directional positions"
            ...
            vol_reason = "Higher than average trading volume confirms strength of price movement - indicates strong institutional participation" if "above" in volume_signal.lower() else "Lower than average volume suggests weak conviction - caution needed for directional moves"
            pdf.add_colored_box("Volume Signal", volume_signal, vol_color, vol_reason)
            
            # Add separator line
            pdf.add_section_separator()
            
            pdf.add_section_header("PRICE POSITIONING IN RANGE")
            
            high_52w = price_data.get('high_52w', 0)
            low_52w = price_data.get('low_52w', 0)
            
            pdf.add_info_pair("52-Week High", f"Rs. {high_52w:.2f}")
            pdf.add_info_pair("52-Week Low", f"Rs. {low_52w:.2f}")
            pdf.add_info_pair("Distance from High", f"{distance_from_high:.2f}%")
            pdf.add_info_pair("Distance from Low", f"{distance_from_low:.2f}%")
            ...
            # Caution Box with expanded disclaimer
...
if __name__ == "__main__":
    try:
        print("\n" + "="*70)
        print("PROFESSIONAL STOCK ANALYSIS SYSTEM".center(70))
        print("="*70)
        
        stock_input = input("\nEnter stock name (Example: RELIANCE, TCS, INFY, WIPRO): ").strip()
        
        if not stock_input:
            print("Error: Stock name cannot be empty")
            exit(1)
        
        analyzer = EnhancedStockAnalyzer(stock_input)
        pdf_path = analyzer.run_complete_analysis()
        
        if pdf_path:
            print(f"SUCCESS: Report saved as '{pdf_path}'\n")
        else:
